...
export default clerkMiddleware(async (auth, req) => {
  ...
  // Improve the subscription check logic to prevent potential redirect loops
  if ((isMealPlanRoute(req) || isProfileRoute(req)) && userId) {
    try {
      // Make a GET request to our internal API
      ...
      if (checkSubRes.ok) {
        ...
        if (data.hasOwnProperty('subscriptionActive') && !data.subscriptionActive) {
          return NextResponse.redirect(new URL("/subscribe", origin));
        }
      } else {
        // Only redirect on specific error conditions
        const errorData = await checkSubRes.json();
